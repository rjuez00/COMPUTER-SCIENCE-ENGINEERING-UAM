...
void escribir_subseccion_data(FILE* fpasm) {
	...
	fprintf(fpasm, "segment .data\n\tmsg_error_division db \"Error: División por 0.\"\n\
	msg_error_indice_vector db \"Error: Indice fuera de rango\"\n");
...
void declarar_variable(FILE* fpasm, char * nombre, int tipo, int tamano) {
	...
	// Useful if we need to add new types
	switch (tipo) {
		case ENTERO:
			type_parsed = 'd';
			break;
		/* case BOOLEANO applies in the default case */
		default:
			type_parsed = 'b';
			break;
	}
	
	fprintf(fpasm, "_%s res%c %d\n", nombre, type_parsed, tamano);
}
