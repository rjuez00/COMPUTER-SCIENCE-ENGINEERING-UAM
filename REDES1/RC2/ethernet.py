...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if (destino != broadcastAddr and  destino != macAddress):
        return
    function = upperProtos.get(struct.unpack("!H", ethertype)[0])
    if function:
        function(us, header, data, origen)
        return
    else:
        return
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    # Añade una entrada nueva al diccionario
...
def startEthernetLevel(interface:str) -> int:
    ...
    if (levelInitialized):
        print('ya inicializado ', interface, 'interface')
        return -1
    errbuff = bytearray()
    macAddress = getHwAddr(interface)
    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, errbuff)
...
def sendEthernetFrame(data:bytes,len:int,etherType:int,dstMac:bytes) -> int:
    ...
    trama = bytearray(dstMac + macAddress + struct.pack("!H", etherType) + data) 
    len += 14
    if(len > ETH_FRAME_MAX):
        return -1
    if(len < ETH_FRAME_MIN):
        off = ETH_FRAME_MIN - len
        trama.extend(bytearray(off))
        len += off
    return pcap_inject(handle, bytes(trama), len)