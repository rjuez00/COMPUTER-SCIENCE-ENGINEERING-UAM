...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    originDataMAC = data[8:14]
    if MAC != originDataMAC:
        logging.debug('Direcciones MAC no coinciden')
        return

    originDataIP = data[14:18]
    destinoDataIP = data[24:28]
    if destinoDataIP == struct.pack("!I", myIP):
        arpreply = createARPReply(struct.unpack("!I", originDataIP)[0], originDataMAC)
        sendEthernetFrame(arpreply, len(arpreply), 0x0806, originDataMAC)
...
def processARPReply(data:bytes,MAC:bytes)->None:
    ...
    originDataIP = data[14:18]
    destinoDataMAC = data[18:24]
    destinoDataIP = data[24:28]

    if destinoDataIP == struct.pack("!I", myIP):
        if struct.unpack("!I", originDataIP)[0] == requestedIP:
            cacheLock.acquire()
            cache[struct.unpack("!I",originDataIP)[0]] = originDataMAC
            cacheLock.release()

            globalLock.acquire()
            resolvedMAC = originDataMAC
            awaitingResponse = False
            requestedIP = None
            globalLock.release()
...
def createARPRequest(ip:int) -> bytes:
    ...
    frame = bytearray( ARPHeader + struct.pack("!H", 0x0001) + myMAC + struct.pack("!I", myIP)+ bytes(6) + struct.pack("!I", ip)  )
...
def createARPReply(IP:int ,MAC:bytes) -> bytes:
    ...
    '''
    global myMAC,myIP
    frame = bytes(ARPHeader + struct.pack("!H", 0x0002) + myMAC + struct.pack("!I", myIP) + MAC + struct.pack("!I", IP))
    return frame
...
def process_arp_frame(us:ctypes.c_void_p, header:pcap_pkthdr, data:bytes, srcMac:bytes) -> None:
    ...
    if data[0:6] != ARPHeader: #  Comprueba el header
        logging.debug('el header ARP no coincide')
        return -1

    opcode = data[6:8]
    if (opcode == struct.pack("!H", 0x0001)):
        processARPRequest(data, srcMac)
    elif(opcode == struct.pack("!H", 0x0002)):
        processARPReply(data, srcMac)
    else:
        logging.debug('opcode no reconocido')
...
def ARPResolution(ip:int) -> bytes:
    ...
    for i in range(0,3):
        ...
        globalLock.acquire()
        if awaitingResponse == False:
            auxiliar = resolvedMAC
            globalLock.release()
            return auxiliar
        globalLock.release()

    logging.debug('respuesta no recibida')
    return None
